...
void sendFile(void)
{
	char* shmaddr;
        ...
        sem_t *sem1 = sem_open("/my_sem1", 0); //named세마포어 open
        ...
        sem_t *sem2 = sem_open("/my_sem2", 0); //named세마포어 open
        if(sem2 == SEM_FAILED)//예외 처리
        {
                perror("sem_open error");
                pthread_exit(1);
        }
	...
	while(1)
	{
		...
		sem_post(sem2);//서버가 Named 세마포어를 해제하기를 기다림
	}
